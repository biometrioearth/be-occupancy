# Load data
y <- readRDS("y_bioacoustic.RDS")  # [39species, 9sampling points, 3 months]
...
#attach names to output files
  ...
  z[z > 1] <- NA
 # plot(z)  
  writeRaster(z, filename = paste0("bioacoustics_occupancy_species_MEAN_", i, ".tif"), overwrite = TRUE)
  ...
  z[z > 1] <- NA
 # plot(z)  
  writeRaster(z, filename = paste0("bioacoustics_occupancy_species_SD_", i, ".tif"), overwrite = TRUE)
  ## DEVIANCES
  deviance_i <- out$k.fold.deviance
  saveRDS(deviance_i, paste0("Bioacoustics_deviance_", i, ".RDS"))
  
  results_df <- rbind(results_df, data.frame(Species = as.character(i), Deviance = deviance_i))

}

#export csv for deviance
write.csv(results_df, file.path("Results_bioacoustics", "Deviance", "bioacoustic_species_deviances.csv"), row.names = FALSE)
